	...
	public static void scheduleMaintenance(Scanner input) throws SQLException, ParseException {
		...
		if (userInput.equals("1")) {
			...
			if (isValidVehicle) {
				...
				int partsavailability = HelperFunctions.checkPartAvailability(ApplicationConstants.MAINTENANCE,
						serviceTobeScheduled, licensePlateNumber, customer.getServiceCenterId());
				System.out.println("Parts availability" + partsavailability);
				if (partsavailability == 0) {
					float duration = HelperFunctions.calculateServiceDuration(ApplicationConstants.MAINTENANCE,
							serviceTobeScheduled, licensePlateNumber);
					...
					scheduleList = SchedulerHelper.getTimeSlot(appointment.getRequestedMechanicFirstName(),
							appointment.getRequestedMechanicLastName(), numOfSlots, customer.getServiceCenterId(),
							nextDate);
					...
					for (int i = 0; i < scheduleList.size(); i++) {
						...
						System.out.println("\t\t " + SchedulerHelper.datetoString(date) + " : " + t.getStartTime()
					...
					if (userInput.equals("1")) {
						...
						int res = statement.executeUpdate("INSERT INTO TIME_SLOT VALUES ('" + index + "', '" + appindex
								+ "', '" + scheduleList.get(counter).getMechanicId() + "', '" + startTime + "', '"
								+ endTime + "')");
					...
					System.out.println(
							"\t The parts required for the service are currently not availabile. Please try scheduling after "
									+ partsavailability
				...
				System.out.println(
						"\t\t******The entered car details could not be found. Please re-enter the car details registered with ACME.******\n\n");
