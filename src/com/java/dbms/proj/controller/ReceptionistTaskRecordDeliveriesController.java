...
public class ReceptionistTaskRecordDeliveriesController {
	public static void recordDeliveries(Scanner input) throws ClassNotFoundException, SQLException, ParseException {
		...
		try {
			...
			tuples = statement.executeUpdate("UPDATE NOTIFICATION SET SHOW = 0 FROM PURCHASE_ORDER PO INNER JOIN NOTIFICATION N WHERE N.ORDER_ID = PO.ORDER_ID AND PO.ORDER_STATUS = 'DELIVERED' AND N.SHOW = 1");	
