...
public class ReceptionistTaskRecordDeliveriesController {
	public static void recordDeliveries(Scanner input) throws ClassNotFoundException, SQLException, ParseException {
		...
		try {
			...
			tuples = statement.executeUpdate("UPDATE NOTIFICATIONS SET SHOW = 0  FROM PURCHASE_ORDER PO INNER JOIN NOTIFICATION N ON N.ORDER_ID = PO.ORDER_ID WHERE PO.ORDER_STATUS = 'DELIVERED' AND N.SHOW = 1");	
